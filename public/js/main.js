...
const emptyStateDiv = document.getElementById('emptySearchState');
...
function filterKeysBySearch() {
    ...
    if (emptyStateDiv) {
        if (visibleCount === 0 && query !== '') emptyStateDiv.classList.remove('hidden');
        else emptyStateDiv.classList.add('hidden');
    }
}
...
const closeReq = document.getElementById('closeModalBtn');
const cancelReq = document.getElementById('cancelModalBtn');
const confirmReq = document.getElementById('confirmSubmitBtn');
const modalName = document.getElementById('modalName');
const modalEmail = document.getElementById('modalEmail');
const modalReason = document.getElementById('modalReason');
const modalPlannedReturn = document.getElementById('modalPlannedReturn');
const modalSelected = document.getElementById('modalSelectedKeysList');
...
function openRequestModal() {
    ...
    modalSelected.innerHTML = '<ul class="list-disc list-inside">' + basket.map(i => `<li>${escapeHtml(i.code)} (${escapeHtml(i.brand)})</li>`).join('') + '</ul>';
    modalName.value = '';
    modalEmail.value = '';
    modalReason.value = '';
    modalPlannedReturn.value = '';
    modalError.classList.add('hidden');
    consentCheckbox.checked = false;
    confirmReq.disabled = true;
...
async function submitRequest() {
    ...
    const planned = modalPlannedReturn.value;
    ...
    modalError.classList.add('hidden');
    confirmReq.disabled = true;
    confirmReq.textContent = 'Submitting...';
    ...
    try {
        ...
        confirmReq.disabled = false;
        confirmReq.textContent = 'Submit request';
    }
}

document.getElementById('submitRequestBtn')?.addEventListener('click', openRequestModal);
closeReq?.addEventListener('click', closeRequestModal);
cancelReq?.addEventListener('click', closeRequestModal);
confirmReq?.addEventListener('click', submitRequest);
consentCheckbox?.addEventListener('change', () => {
    confirmReq.disabled = !consentCheckbox.checked;
});
...
const returnStep1Div = document.getElementById('returnStep1');
if (returnStep1Div) {
    ...
    returnStep1Div.insertBefore(emailInput, returnStep1Div.querySelector('p').nextSibling);
}

const reportLostModal = document.getElementById('reportLostModal');
const reportLostEmailInput = document.getElementById('reportLostEmailInput');
const fetchLostReportKeysBtn = document.getElementById('fetchLostReportKeysBtn');
...
function openReportLostModal() {
    ...
    if (reportLostEmailInput) reportLostEmailInput.value = '';
...
async function loadLostReportKeys(email) {
    ...
    try {
        ...
        document.querySelectorAll('.reportLostFromListBtn').forEach(btn => {
            btn.addEventListener('click', () => {
                showLostConfirmModal(parseInt(btn.dataset.transactionId), btn.dataset.keyCode);
            });
        });
    } catch (err) {
...
fetchLostReportKeysBtn?.addEventListener('click', () => {
    ...
    loadLostReportKeys(email);
});

let pendingLostTransaction = null;

function showLostConfirmModal(transactionId, keyCode) {
    pendingLostTransaction = { id: transactionId, code: keyCode };
...
async function executeReportLost() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const emailInput = document.getElementById('reportLostEmailInput');
            const email = emailInput?.value.trim();
            if (email) await loadLostReportKeys(email);
            await fetchKeys();
...
document.getElementById('confirmLostBtn')?.addEventListener('click', executeReportLost);
    ...
    try {
        ...
        document.querySelectorAll('.extension-item').forEach(item => {
            item.addEventListener('click', function() {
                ...
                selectedLoanId = parseInt(this.dataset.transactionId);
        ...
        document.getElementById('submitExtensionBtn').dataset.loanId = '';
        document.querySelectorAll('.extension-item').forEach(item => {
            item.addEventListener('click', function() {
...
document.getElementById('submitExtensionBtn')?.addEventListener('click', async function() {
    ...
    if (!loanId) {
        document.getElementById('extensionError').textContent = 'Please select a loan to extend.';
        document.getElementById('extensionError').classList.remove('hidden');
        return;
    }

    if (!newReturnDate) {
        document.getElementById('extensionError').textContent = 'Please select a new return date.';
        document.getElementById('extensionError').classList.remove('hidden');
        return;
    }
    ...
    try {
        ...
        document.getElementById('extensionError').textContent = err.message;
        document.getElementById('extensionError').classList.remove('hidden');
    } finally {
        this.disabled = false;
