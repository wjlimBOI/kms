    ...
    for (const item of items) {
      const result = await client.query(
        `INSERT INTO transactions
         ...
         VALUES ($1, $2, 'borrow', $3, $4, $5, NOW(), 'borrowed', 'admin_approval', $6, 'admin_approval', $7, $8, $9) RETURNING *`,
      ...
      insertedTransactions.push(newTrans);
      await logInsert({
        ...
        req,
        extraDetails: { action: 'approve_request_transaction', request_id }
      });
    }
...
router.post('/extend', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (txResult.rowCount === 0) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: 'Transaction not found, not borrowed, or email mismatch' });
    }
    ...
    await client.query('COMMIT');
    res.json({ message: 'Extension request submitted successfully. New return date updated.', transaction: newTx });
  } catch (err) {
    await client.query('ROLLBACK');
