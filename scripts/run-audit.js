...
async function validateAuditChain() {
    ...
    try {
        console.log(`[${new Date().toISOString()}] 🔍 Running audit chain validation...`);
...
async function main() {
    try {
        ...
        if (result.firstTamperedId) {
            console.log(`First Tampered: ${result.firstTamperedId}`);
            console.log(`Total Tampered: ${result.tamperedIds.length}`);
        }
        console.log('─────────────────────────────');

        if (result.status === 'tampered') {
            console.error('⚠️  AUDIT CHAIN TAMPERED!');
            process.exit(1);
        } else if (result.status === 'error') {
            console.error('❌ Validation error occurred');
            process.exit(2);
        } else {
            console.log('✅ Audit chain is healthy');
            process.exit(0);
        }
    } catch (err) {
